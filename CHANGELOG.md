...
本版本修复 Windows 上启动或停止 Android 屏幕镜像时可能发生的原生闪退。

### 修复
- 调整 Windows 原生镜像纹理生命周期：先停止解码线程、注销 Flutter 纹理，再销毁纹理对象
- 避免镜像启动失败或清理会话时的竞态和 use-after-free 访问冲突

### 验证
- Android / iOS 镜像控制器与镜像页面回归测试通过
- Windows x64 Release 构建通过
- 安装包继续使用相同 AppId，支持静默覆盖升级
